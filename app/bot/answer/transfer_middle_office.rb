def transfer_middle_office(session_id, sender, msg = "", type)
  ...
  url = "https://mymessagingstore.herokuapp.com/api/v1/sessions?bot_id=#{ENV['mms_token']}&key=#{key}&fbid=#{sender_id}&msg=#{msg}&first_name=#{username}&sender=#{type}&context=#{session.context}&profile_picture=#{profile_picture}"
